...
class AgentPromptTests(unittest.TestCase):
    def test_load_agent_prompt_appends_accuracy_contract(self) -> None:
        ...
        self.assertIn("trace each `exit`", prompt)
        self.assertIn("uses `bash -e`", prompt)
        self.assertIn("executes `exit`", prompt)
        self.assertIn("process.argv[1]", prompt)
        self.assertIn("YAML removes the block indentation", prompt)
        self.assertIn("can finish green after a later notification", prompt)
