...
import { WorkspaceScopedResource, scopePlatformClient } from './base.js'
...
/** Build text conversation WebSocket URLs. */
export class ConversationWebSocketsResource extends WorkspaceScopedResource {
  constructor(
  ...
  override withOptions(options: ScopedRequestOptions): this {
    return new ConversationWebSocketsResource(
   ...
   * @example
   * ```ts
   * const url = client.conversationWebSockets.sessionConnectUrl({
...
export function textStreamAuthProtocols(apiKey: string): TextStreamAuthProtocols {
  ...
  if (!WEB_SOCKET_PROTOCOL_TOKEN_RE.test(token)) {
    ...
    throw new ConfigurationError(
      `apiKey contains characters browsers reject in WebSocket subprotocols (${invalidChars}); use the token option on client.conversationWebSockets.textStreamUrl() instead for keys containing these characters, only in trusted contexts where URLs are not logged in browser history, server access logs, HTTP proxy logs, or referrer headers`,
    )
  }
